...
def make_stat():
    ...
    for timestamp in all_sessions:
        ...
        for session in serv_timestamp.smembers(timestamp):
            ...
            try:
                dic_cmd_overtime[str(command_after_log)] += [timestamp]
            except KeyError:
                dic_cmd_overtime[str(command_after_log)] = [timestamp]


    to_write = { 'overtime': str(dic_cmd_overtime),
    ...
    print({'tot_number_of_session': tot_ip_num, 
            'invalid_session': invalid_session, 
            'session_without_command': session_without_cmd})
    with open("make_stats_telnet_commands_after_log_overtime.output", 'w') as f:
